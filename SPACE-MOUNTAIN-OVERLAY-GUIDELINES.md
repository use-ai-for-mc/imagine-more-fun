...
- **Master gate — `SpaceMountainOverride.isActive()`.** True only when `BAKING_MODE` is off, the "Space Mountain Enhancements" config toggle is on, the client is connected to ImagineFun, and the current ride is `SPACE_MOUNTAIN` or `HYPERSPACE_MOUNTAIN`. Every overlay — block overlay, launch tunnel, ride audio, and the disco-ball starfield — routes through it. Single-player and other servers fail the ImagineFun check, so nothing activates there.
