...
Registered in `ImfClient.onInitializeClient()` in this order: `SpaceMountainStarRenderer`, `SpaceMountainTrackRenderer`, `SpaceMountainBlockOverride.init()`, `SpaceMountainTunnelRenderer`, `SpaceMountainEntryTunnelSeal`, `SpaceMountainDiscoBall`, `SpaceMountainRideAudio`. (`CoasterTiltAmplifier` needs no registration — it's driven by the `NraSmoothCoastersRotationMixin` interceptor.)
...
Two independent things bank: the **rendered rails** (from the baked SM/HSM track) and the **camera** (by scaling SmoothCoasters' live tilt — now a global multiplier across all coasters). They no longer share a roll source.

The vehicle recorder only ever captured yaw/pitch, so the baked track was cross-sectionally flat. `dome_track.bin` is now **IFTC v2**: every sample carries a signed `roll` (bank) angle, computed offline from path curvature by `~/imf-debug-dumps-archive/bake-roll.py` — `roll = clamp(BANK_SCALE · atan(v·ω/g), ±BANK_CAP)`, tuned (`BANK_SCALE=0.55`, `BANK_CAP=38°`) to the ~30-40° a real coaster banks. `bake-roll.py` reads the existing bin and only *adds* the roll column, so the recorded geometry never moves. This roll now feeds **only** the rail geometry.

- `CoasterTrackData` parses `dome_track.bin` (v1 *or* v2 — v1 → roll all zeros) once into shared arrays; `nearestSample(x,y,z)` is a full 3D scan. Consumed only by the track renderer now.
...
- **Camera bank is no longer baked.** SmoothCoasters tilts the camera on every coaster it covers — it funnels every server rotation packet through `SmoothCoasters.setRotation(Quaternionfc target, int ticks)` (the single funnel across all protocol versions; V4/V5 inherit V6's rotation handler) before interpolating the pose onto the camera. `NraSmoothCoastersRotationMixin` (`@Pseudo`, soft-dep on SmoothCoasters) intercepts that target pose at HEAD and hands it to `CoasterTiltAmplifier.amplify`, which measures the pose's bank geometrically (world-up projected ⊥ the seat-forward axis → how far the seat's up has rolled about forward) and rotates the pose about its world-space forward axis by `(multiplier − 1) × bank`, so the total becomes `multiplier × bank`. SmoothCoasters' own interpolation + camera write then carry the adjusted lean — no per-tick track lookup, no EMA, always in sync with the real ride. **Roll only**: heading (yaw) and slope (pitch — drops/climbs) are left exactly as the server sent them.
- ImagineFun-gated only (consistent with the rest of the mod); applies to all SC-tilted coasters, not just SM/HSM. A multiplier of 1.0 passes the pose through untouched (and < 1.0 *reduces* the lean — a comfort knob). Replaced the old `CoasterCameraBank` + `NraCameraRollMixin` baked-track, position-sampled pair (removed 2026-05-26); the SM/HSM-only "Additional Tilt" was generalized to this global "Coaster Tilt" multiplier the same day.

**Config** (Modifications, top-level — above the Space Mountain group): **Coaster Tilt** slider (`coasterTiltMultiplier`, double 0.0–2.0, default 1.0; the UI is an int 0–200 ÷100, shown as e.g. `1.50×`). 1.00× = stock SmoothCoasters tilt, 2.00× = double, 0.00× = level. Scales only the *camera* — the SM/HSM rails bank regardless. Read live, so the in-game slider is the tuning knob.
